...
def main_menu_kb() -> InlineKeyboardMarkup:
    ...
    # ВНИМАНИЕ: кнопка «🧰 Промты (библиотека)» НЕ добавляется сюда — она
    # видна только в потоке массовой генерации (`mass_list_kb`). Так
    # индивидуальный поток не показывает юзеру лишний UX. Команда
    # `/prompts` остаётся доступной по тексту, но не светится в /menu.
