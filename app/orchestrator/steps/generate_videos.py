    ...
    async with browser_session() as bs:
        ...
        try:
            for fr in frames:
                ...
                #
                # `cancel_check` пробрасывается внутрь `_wait_video_url` —
                # без этого «⏹ Остановить» срабатывал бы только между
                # кадрами, а внутри одного кадра юзер либо ждёт до 20 минут,
                # либо переживает «жму стоп но ничего не происходит».
            ...
            # Приводим к тому же виду, что «между-кадровая» отмена: сбрасываем
            # флаг (если вдруг всё ещё взведён) и конвертируем в StepCancelledError.
            from app.services.step_cancel import consume_stop

            consume_stop(project.id)
            logger.info(
                "[#{}] generate_videos: проект #{}: отмена во время ожидания "
