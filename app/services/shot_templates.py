...
# T1 = взаимодействие двоих (не только «говорят»: передают/принимают/
# приказывают — тоже T1). T5 = только ДЛИТЕЛЬНАЯ работа руками (процесс).
...
_RE_T1 = (
    ...
    r"рассказал|переда(ёт|ет|ют|л|ла)|вруча|принос|"
    r"принима(ет|ют|л|ла)[^.\n]{0,30}\s(от|у)\s|"
    r"приказыва|приказал|приветств|ссор|выслушива|выслушал|требу(ет|ют) с|"
    r"требу(ет|ют) запис|отворачива|протягива(ет|ют)"
)
_RE_T2_VERB = (
    ...
    r"открывает|открывают|раскрыл|раскрыва(ет|ют)|поднял|вытащил|изуча|изучил"
)
_RE_T2_ITEM = (
...
_RE_T8 = (
    ...
    r"эпох|в\s(18|19|20)\d\d|детств|юност|молодост|карьер|принадлеж|"
    r"владел|владения|перечисл"
)
_RE_T4 = (
        ...
        for tid, cnt in sorted(counts.items()):
            if cnt >= 3 and tid != "T8":
                lines.append(
                    ...
                    "заново: «передают/приказывают» = T1, «раскрывает/читает "
                    "предмет» = T2, «указывает/показывает» = T4, «получил/"
                    "услышал» = T7; T5 — только длительная работа руками."
                )
    if not any_row:
        return ""
    return "\n".join(lines).strip() + "\n"
