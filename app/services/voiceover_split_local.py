...
def assert_frames_spec_within_limits(
    frames_spec: list[dict],
    *,
    min_chars: int | None,
    max_chars: int | None,
) -> None:
    """Fail-closed: если лимиты заданы — все ячейки должны попасть в диапазон.

    Не переписывает текст (никакого auto-enforce). Нарушение → RuntimeError,
    кадры в БД не пишутся.
    """
    ...
    if bad:
        ...
        raise RuntimeError(
            "split_db: GPT-разбивка вне лимитов "
            f"{lo}-{hi}: outside={len(bad)}/{st.get('n')} "
            f"min/avg/max={st.get('min')}/{st.get('avg')}/{st.get('max')} "
            f"sample_lens={bad[:8]}. "
            "Нода не готова — поправь промт/настройки или перезапусти ▶."
        )
