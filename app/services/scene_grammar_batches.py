...
У КАЖДОГО shot обязательны: роль, особенность_сцены (из словаря крупностей),
акцент (уникальный объект/жест ЭТОГО шота), действие, описание_кадра,
логика_перехода, фон, предметы.
Запрещено: одинаковый акцент на двух shots; копировать scene.акцент на все shots;
одинаковая особенность_сцены подряд без причины; абстрактный акцент («напряжение»).
Верни ТОЛЬКО:
