...
_BODY_RE = re.compile(
    ...
    r"подошла|снял|надева|переда|протяж|душ|хват|отпуска|удар|бьёт|бьет",
    re.IGNORECASE,
)
_ITEM_RE = re.compile(
    r"открыл|открыва|взял|бер[её]т|папк|папк[уие]|печат|ключ|документ|"
    r"письм|жалоб|достал|доста[её]т|положил|перчат|улик|карман|расклад|"
    r"носок|кольц",
    re.IGNORECASE,
)
_FACE_RE = re.compile(
    r"лицо|крупн|профиль|глаз|прочитал|понял|поняла|взглянул на лицо|"
    r"поднял (глаза|голову)|испуга|усмех",
    re.IGNORECASE,
)
_PAIR_RE = re.compile(
...
_VISIBLE_VERB_RE = re.compile(
    ...
    r"протяж|душ|хват|отпуска|расклад|показ",
    re.IGNORECASE,
)
