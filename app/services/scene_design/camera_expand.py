...
def assign_parent_vo_parts(members: list[Any], full: str) -> int:
    """Записать фрагменты полного закадра ячейки на шоты группы. Возвращает число изменений."""
    if not members:
        return 0
    parts = split_text_into_parts(full, len(members))
    changed = 0
    for fr, part in zip(members, parts):
        prev = str(getattr(fr, "voiceover_text", None) or "").strip()
        if prev != part:
...
async def resplit_vo_fragments_from_parents(
    session: AsyncSession,
    project: Project,
    *,
    parent_uuids: set[str] | None = None,
) -> int:
    """После сценариста: полный закадр на родителе снова режется по шотам.

    GPT пишет целую ячейку на uuid родителя; дети до этого держат старые
    фрагменты. Без resplit меню снова покажет полный текст только на первом
    шоте и старые куски на остальных.
    """
    frames = list(
        (
            await session.execute(
    ...
    for fr in frames:
        meta = _subdivide_attr(fr)
        pu = str(meta.get("parent_uuid") or "").strip() or str(fr.uuid or "").strip()
        if not pu:
            continue
        by_parent.setdefault(pu, []).append(fr)
    allowed = {str(u).strip() for u in (parent_uuids or set()) if str(u).strip()}
    changed = 0
    for members in by_parent.values():
        if len(members) < 2:
            continue
        ...
        if parent_fr is None:
            continue
        parent_uid = str(parent_fr.uuid or "").strip()
        if allowed and parent_uid not in allowed:
            continue
        full = (parent_fr.voiceover_text or "").strip()
        if not full:
            continue
        changed += assign_parent_vo_parts(members, full)
    if changed:
        await session.flush()
        logger.info(
            "[#{}] resplit VO after script_writer: {} frames",
            project.id,
            changed,
        )
    return changed
