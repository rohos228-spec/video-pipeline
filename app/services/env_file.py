"""Чтение локального .env: последний непустой ключ побеждает.

python-dotenv / pydantic берут последнюю строку как есть. Пустой дубль
`KIE_API_KEY=` ниже по файлу затирает рабочий ключ выше — Create тогда
думает, что ключа нет. Здесь пустые присваивания пропускаем.
"""
...
def last_nonempty_dotenv_values(path: Path, names: tuple[str, ...] | list[str]) -> dict[str, str]:
    """Последнее непустое `NAME=value` для каждого имени (без учёта регистра)."""
