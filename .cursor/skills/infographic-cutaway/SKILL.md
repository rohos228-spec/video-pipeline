...
~20°. Плоские заливки, контур 0.5pt, тень под срезом. Подписи 1–3 слова на
выносках с красной точкой, легенда, врезка с шагами, эталон масштаба.
...
| Разрез | здание, нора, дерево | срез + 2–3 врезки |
| Стадии | процесс, рецепт | 4–7 кадров во времени |
...
| Стол | `printed sheet lying flat on a dark wooden desk seen from above, a few [РЕКВИЗИТ] beside it; sheet perfectly flat and unwarped` |
...
Первый — бумага, второй — контур. Точки #D0021B.
...
- Обязательно: `each label appears exactly once`, `lettering crisp, no gibberish`.
- Референс-картинку не прикладывай — перебивает тему: Титаник плюс текст про
  самолёт даёт пять кораблей.
...
- Врезка: кружок с цифрой + 3 строки 7pt. Выноска: излом, точка 1.5px.
...
Скобок не осталось, `PROMPT` ≤ 4700 знаков, подписи на одном языке и по бюджету,
в NEGATIVE запрещена чужая эпоха. Выдай `PROMPT`, `NEGATIVE PROMPT`, `ASPECT`. Примеры —
