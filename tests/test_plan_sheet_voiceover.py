...
from app.storage.plan_sheet_v8 import plan_frame_column, read_plan_voiceover, read_plan_voiceover_cells
...
def test_read_plan_voiceover_cells_bulk(tmp_path: Path, monkeypatch) -> None:
    root = tmp_path / "data"
    slug_dir = root / "videos" / "t1"
    slug_dir.mkdir(parents=True)
    xlsx = slug_dir / "project.xlsx"
    ...
    ws = wb.active
    ws.title = SHEET_PLAN_V8
    ws.cell(row=ROW_VOICEOVER_V8, column=plan_frame_column(1), value="Кадр один")
    ws.cell(row=ROW_VOICEOVER_V8, column=plan_frame_column(2), value="Кадр два")
    wb.save(xlsx)

    monkeypatch.setattr("app.models.settings.data_dir", root)
    project = Project(topic="t", slug="t1")

    assert read_plan_voiceover_cells(project, [1, 2]) == [
        (1, "Кадр один"),
        (2, "Кадр два"),
    ]
