...
def test_kadry_from_bits_covers_full_vo() -> None:
    ...
    bits = [
        ...
        },
        {
            ...
            "закадр": "Для следствия он оставался неизвестным преступником.",
        },
        {
            "порядок": 3,
            "глагол": "рождается",
...
def test_kadry_from_bits_empty_when_chunks_miss_vo() -> None:
    from app.services.vo_shot_expand import kadry_from_bits

    vo = "Он вошёл в кабинет. Отец остался у двери."
    planned = kadry_from_bits(
        vo,
        [
            {"порядок": 1, "закадр": "Он вошёл в кабинет."},
            {"порядок": 2, "закадр": "чужой текст"},
        ],
    )
    assert planned == []
...
def test_bit_kadry_do_not_collapse_as_coverage() -> None:
    ...
    bits = [
        {"порядок": 1, "закадр": "Первая фраза целиком."},
        {"порядок": 2, "закадр": "Вторая тоже на месте."},
        {"порядок": 3, "закадр": "Третья закрывает."},
    ]
    planned = kadry_from_bits(vo, bits)
