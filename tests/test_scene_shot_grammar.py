...
def test_bit_spans_keep_vo_chunks() -> None:
    ...
    bits = fill_bit_spans(
        ...
        [
            ...
            },
            {
                ...
                "закадр": "сел к столу и",
            },
            {
                "порядок": 3,
                "изменение": "папка закрыта → открыта",
                "закадр": "открыл папку с жалобой.",
            },
        ],
    )
    assert bits_cover_vo(vo, bits)
    assert "якорь" not in bits[0]
    assert "вошёл" in bits[0]["закадр"]
    assert "открыл" in bits[-1]["закадр"]


def test_bit_spans_drop_anchor_and_reject_glue_gap() -> None:
        ...
        [
            {
                ...
                "якорь": "Он вошёл",
                "закадр": "Он вошёл в кабинет.",
            },
            {
                "порядок": 2,
                "глагол": "оставаясь / не входя",
                "якорь": "Отец остался",
                "закадр": "Отец остался у двери.",
            },
        ],
    )
    assert "якорь" not in bits[0]
    assert "якорь" not in bits[1]
    assert bits_cover_vo(vo, bits)
    broken = fill_bit_spans(
        vo,
        [
            {"порядок": 1, "закадр": "Он вошёл в кабинет."},
            {"порядок": 2, "закадр": "дверь закрыта"},
        ],
    )
    assert not bits_cover_vo(vo, broken)
