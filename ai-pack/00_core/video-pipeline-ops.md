...
## anim_pr
- Источник правды: **DB** (`Frame.animation_prompt` / apply-ops).
  Excel R48 — только после явного Export (`excel_io` / кнопка); auto sync R48→DB отключён.
...
## Excel I/O
- Runtime: только `app/services/excel_io.py` (`import_project_xlsx` / `export_project_xlsx`).
- `apply_ops` по умолчанию **не** пишет xlsx; База не auto-export.
