...
Рабочие ветки машин: **`housepc`**, **`tompc`**, **`strangepc`**, **`workpc`**.
Целевая ветка задаётся локально: `ORCHESTRATOR_GIT_BRANCH` в `.env` (этот ПК = `housepc`).
...
- пушить с housepc в `main` / чужую ПК-ветку (если пользователь не сказал явно)
...
Слияние ПК-веток в `main` — отдельной командой пользователя.
...
- Ветка этого ПК: `housepc` (`ORCHESTRATOR_GIT_BRANCH=housepc`)
...
## anim_pr
- Источник правды для пропуска/генерации: **колонка R48 в project.xlsx**, не stale DB
- `animation_prompt_gpt.py`: `sync_animation_prompts_from_xlsx`, `scan_missing_animation_prompts`
- Soft retry без wipe: `step_failure_policy._SOFT_RETRY_STEP_CODES` включает `anim_pr`, `img`, `video`
