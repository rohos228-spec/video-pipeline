...
- **Локации данных**: SQLite (файл `./data/state.db`) + `./data/videos/<slug>/{characters,scenes,videos,audio,subs,final}/`.  
  Решено убрать Docker/Postgres в пользу SQLite + pure Python по требованию пользователя — проще развёртывание на Windows.
...
seed_pilot.py              — CLI для засева пилотного проекта («5 фактов о рачках в стиле киберпанк»)
...
1. **Мёрдж PR #5** (который содержит SQLite/no-Docker перевод и пилотный сид «рачки») поверх PR #1-4.
   PR #1-4 уже включают весь пайплайн; PR #5 делает его запускаемым без Docker.
