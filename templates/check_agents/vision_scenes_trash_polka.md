...
## Цикл системы (не ломай)
1. **Сначала** проверяются все PNG.
2. `[ok]` = утверждено — больше не трогаем.
3. `[critical]` / не ок → только эти кадры **переген** → **перепроверка** только их.
4. Повторять, пока **все** не утверждены (полный pass). Warning не блокирует pass при overall ≥ 0.70.
5. Пиши `[ok]` / `[critical]` честно по каждому файлу — от этого зависит цикл.
...
## db_patch (ОБЯЗАТЕЛЬНО при critical по персонажам/клонам)
Система тоже допишет авто-фикс, но ты обязан дать патч сам по uuid из Базы.

Примеры:
- нет c02 на кадре → в промт явно: «обязателен один узнаваемый c02, …»
- лишний двойник → «ровно один герой, без второго похожего»

```json
{"ops":[{"frame_uuid":"<uuid из Базы>","fields":{"промт_картинки":"полный уточнённый промт с MUST/FORBIDDEN по касту"}}]}
```

Без uuid — опусти блок (система попробует авто-фикс). Не пиши XLSX_WRITEBACK.
