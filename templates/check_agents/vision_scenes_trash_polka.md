...
1. **Сначала** проверяются все PNG пачки.
2. При `[critical]` + `frames:` — только эти кадры удаляются и **перегенерируются**.
3. Потом check снова, но **только переделанные** PNG (остальные уже приняты).
4. Макс. 3 раунда. Пиши critical честно — от этого зависит regen.
