...
| Ячейка | Порядок во времени | Смысл |
|--------|--------------------|--------|
| 1 | старт (~0–⅙) | завязка / стартовая композиция |
| 2 | | развитие |
| 3 | середина | |
| 4 | | |
| 5 | | |
| 6 | финал (~⅚–конец) | итоговая поза / камера |

Ссылайся: `video_sheet_009_….png` / `frame_009` / `9` + ячейка `cell:3`.  
Разбирай **последовательность 1→6**: логика движения, камера, смысл сцены.  
