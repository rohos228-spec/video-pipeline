...
export function MassFactoryPanel({ project }: { project: ProjectDetail }) {
  ...
  const statusUnavailable = status.isError && localTopics.length === 0;
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-violet-500/25 bg-violet-500/5 p-2.5">
      ...
      <p className="text-[10px] leading-snug text-muted-foreground">
        ...
        Сначала «Excel тем», затем «Запустить очередь». Перезапустите backend после
        git pull, если видите API error.
      </p>

      {statusUnavailable ? (
        <p className="text-[10px] text-amber-400/90">
          Backend устарел или не перезапущен — загрузите Excel заново или обновите
          через UPDATE-STUDIO.cmd
        </p>
      ) : null}
