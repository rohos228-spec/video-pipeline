...
### Сценарий C. Оркестратор (цель, в подключении)

Пишешь оркестратору по-русски: «перепиши закадр во 2 кадре на …»,
«сделай промты видео динамичнее». Он сам: контекст из графа →
JSON apply-ops → запись → harness-проверка. Пока ноды подключаются —
пользуйся сценариями A/B.
