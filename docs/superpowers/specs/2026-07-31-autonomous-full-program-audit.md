...
Фоновые карты под-агентов завершены и сведены в `2026-07-31-context-pack.md`: orchestrator, Excel/v8, LLM/GPT, media generators, runtime/UI/harness, UI smoke #50. По их находкам закрыты follow-up баги: harness R48 status typo `generating_video`→`generating_videos`, подсчёт R45/R48/R49 теперь идёт до `ws.max_column` (не режет >17 кадров), в `gpt_operator_client` исправлен format-retry `project_file` (`WRITEBACK_HINT` был использован без импорта → NameError вместо чистого retry). UI smoke #50: PASS по безопасному чеклисту; мелкий backlog — первый клик «Логи» иногда даёт `Failed to fetch logs`, рассинхрон label ноды plan, V-меню уходит за верхний край, Create Outsee при балансе $0.00 нужен явный баннер.
