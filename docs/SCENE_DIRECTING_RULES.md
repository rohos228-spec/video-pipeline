...
| fw_action | карточка `N.` на каждый якорь по промту оператора (без него — один шаг на якорь) + паспорт | `main_action_from_bits_ru` |
| fw_shots | шаг → кадр: `якорь_n`, роль, объект, план, ракурс, движение, стык | `scenes_to_frames_ru`; нет ответа → таблица камеры |
...
- Бюджет: один кадр на якорь. Больше — только если промт оператора сам
  режет шаги через `→`. Сверх бюджета код отрезает лишние шаги.
