...
## Первая установка на новый ПК (в 1 клик)

1. Положите файл `.env` с вашими API-ключами в папку с проектом (или рядом с `SETUP.cmd`).
2. **Дважды кликните `SETUP.cmd`**.
3. Скрипт сам настроит окружение за ~30–45 секунд и создаст ярлык **«Video Pipeline Studio»** на Рабочем столе.

Дальнейший повседневный запуск — просто клик по ярлыку на Рабочем столе или **`STUDIO.cmd` → `1`**.
